...
const postWebhook = trigger({
  ...
  output: [{ headers: { authorization: 'Bearer token' }, body: { projectId: 'project-id', projectName: 'ShopSmart', documentType: 'test_strategy', productOwner: 'PO', environment: 'local' } }],
});
...
const prepareRequest = node({
  ...
  version: 2,
  config: {
    name: 'Prepare Professional Queue Request',
    parameters: {
...
return [{
  ...
  output: [{ ok: true, jobId: 'PRO-260512-ABC123', retryMode: false, token: 'token', projectId: 'project-id', environment: 'local', input: { projectName: 'ShopSmart', documentType: 'test_strategy', generatorMode: 'professional' } }],
});
...
const prepareRuntimeRequest = node({
  ...
  version: 2,
  config: {
    name: 'Prepare Runtime Config Request',
    parameters: {
...
return [{
  ...
  output: [{ ok: true, jobId: 'PRO-260512-ABC123', retryMode: false, requestedBy: 'qops-user-id', runtimeRequest: { p_environment_key: 'local', p_project_id: 'project-id', p_pipeline: 'generation', p_requested_by: 'qops-user-id' } }],
});

const runtimeReady = ifElse({
  version: 2.3,
  config: {
    name: 'Runtime Request Ready?',
    parameters: {
      conditions: {
        combinator: 'and',
        options: { caseSensitive: true, leftValue: '', typeValidation: 'strict', version: 3 },
...
return [{ json: { ...job, settingsVersion, configSnapshot } }];`,
  ...
  output: [{ jobId: 'PRO-260512-ABC123', retryMode: false, projectId: 'project-id', requestedBy: 'qops-user-id', settingsVersion: 1, configSnapshot: { jira: { projectKey: 'KAN' } }, input: { projectName: 'ShopSmart', documentType: 'test_strategy', generatorMode: 'professional' } }],
});
...
const persistJob = node({
  ...
  output: [{ job_id: 'PRO-260512-ABC123', status: 'pending' }],
});

const jobPersisted = ifElse({
  version: 2.3,
  config: {
    name: 'Professional Job Persisted?',
    parameters: {
      conditions: {
        combinator: 'and',
        options: { caseSensitive: true, leftValue: '', typeValidation: 'strict', version: 3 },
...
const logQueued = node({
  ...
  version: 4.4,
  config: {
    name: 'LOG: Professional Job Queued',
    parameters: {
      method: 'POST',
...
const respondRetryUnavailable = node({
  ...
  version: 1.5,
  config: {
    name: 'Respond Professional Retry Unavailable',
    parameters: {
      respondWith: 'json',
...
export default workflow('pro-qa-generation-queue-creator-ready-draft', 'PRO QA Generation Queue Creator - Ready Draft')
  .add(postWebhook)
  .to(prepareRequest)
