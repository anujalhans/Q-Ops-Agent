# INGEST Worker Engine (Queue Processor)
...
| Field | Value |
| --- | --- |
| Workflow ID | iKOec9hKQmR2KgHs |
| Active | False |
| Archived | False |
| Created At | 2026-04-14T09:41:25.246Z |
| Updated At | 2026-05-07T05:15:37.764Z |
| Node Count | 13 |
| JSON Source | C:\Users\anujalhans01\Q-Ops_Agent\docs\n8n_workflows_2026-05-18_171444\Unpublished\INGEST Worker Engine (Queue Processor) [iKOec9hKQmR2KgHs].json |
...
- https://ifnznfspkjayhnooncrv.supabase.co/rest/v1/doc_ingestion_jobs?status=eq.pending&order=created_at.asc&limit=1
...
- Convert Files Object â†’ Array -> Download Files (Convert URL â†’ Binary) (source output 0, target input 0)
- Download Files (Convert URL â†’ Binary) -> Convert ALL binaries inside ONE item (source output 0, target input 0)
- Convert ALL binaries inside ONE item -> Call 'Multimodal Knowledge Ingestion & Vectorization Engine' (source output 0, target input 0)
- LOG -> Store LOGS in Supabase (source output 0, target input 0)
- Merge -> Convert Files Object â†’ Array (source output 0, target input 0)
- Store LOGS in Supabase -> Merge (source output 0, target input 1)

## Nodes

### Call 'Multimodal Knowledge Ingestion & Vectorization Engine'
...
- Convert ALL binaries inside ONE item -> Call 'Multimodal Knowledge Ingestion & Vectorization Engine' (output 0, input 0)
...
```json
{
    "workflowId":  {
                       "__rl":  true,
                       "value":  "n0fvS28StF5iMZvG",
                       "mode":  "list",
                       "cachedResultUrl":  "/workflow/n0fvS28StF5iMZvG",
                       "cachedResultName":  "Multimodal Knowledge Ingestion \u0026 Vectorization Engine"
                   },
    "workflowInputs":  {
...
- Download Files (Convert URL â†’ Binary) -> Convert ALL binaries inside ONE item (output 0, input 0)
...
- Merge -> Convert Files Object â†’ Array (output 0, input 0)

**Outgoing Connections**

- Convert Files Object â†’ Array -> Download Files (Convert URL â†’ Binary) (output 0, input 0)
...
### Download Files (Convert URL â†’ Binary)
...
```json
{
    "url":  "https://ifnznfspkjayhnooncrv.supabase.co/rest/v1/doc_ingestion_jobs?status=eq.pending\u0026order=created_at.asc\u0026limit=1",
    "authentication":  "genericCredentialType",
    "genericAuthType":  "httpCustomAuth",
    "sendHeaders":  true,
    "specifyHeaders":  "json",
