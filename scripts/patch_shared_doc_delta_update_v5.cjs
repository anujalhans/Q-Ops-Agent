...
const mergeExistingConfluenceExpressionV5 = String.raw`={{ (() => {
  ...
  const compactEnough = (sectionHtml) => stripTags(sectionHtml).split(/\s+/).filter(Boolean).length >= 12;
  ...
  const normalizeTablesForConfluence = (html) => String(html || '').replace(/<table\b[^>]*>[\s\S]*?<\/table>/gi, (table) => {
    ...
    const headerCount = (headerMatch[0].match(/<th\b[^>]*>/gi) || []).length || (headerMatch[0].match(/<td\b[^>]*>/gi) || []).length;
  ...
  const missingClaimed = claimedPreserved
    ...
    .filter(section => !finalParts.sections.has(sectionKey(section)));
  if (missingClaimed.length) {
    throw new Error('Shared update merge guard failed: preserved section(s) missing from final body: ' + missingClaimed.join(', '));
  }
...
function patchPromptLibrary(node) {
  ...
  code = code.replace('SHARED_DELTA_UPDATE_V2', 'SHARED_DELTA_UPDATE_V5');
  code = code.replace('SHARED_DELTA_UPDATE_V4', 'SHARED_DELTA_UPDATE_V5');
  code = code.replace(
...
function patchUpdatePage(node) {
  ...
  htmlParam.value = mergeExistingConfluenceExpressionV5;
}
...
async function main() {
  ...
  try {
    ...
    const backupPath = path.join(backupDir, `workflow_${workflowId}_before_shared_doc_delta_update_v5_${stamp}.json`);
    ...
    console.log(JSON.stringify({
      ...
      backupPath,
      patched: [
        'shared delta update V5 prompt instructions',
        ...
        'section-aware final body merge for shared update only',
        'update-mode table normalization and truncated-table guard',
        'pre-publish guards for duplicate headers, missing preserved sections, incomplete documents, truncated output, malformed tables, and dropped coverage'
      ]
    }, null, 2));
  } finally {
    db.close();
