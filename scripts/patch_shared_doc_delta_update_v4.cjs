...
const mergeExistingConfluenceExpressionV4 = String.raw`={{ (() => {
  ...
  body = sanitizeUserFacingHtml(body || '').trim();
  const finalHtml = buildSummaryHtml() + '<hr/>' + body;
...
function patchPromptLibrary(node) {
  ...
  code = code.replace('SHARED_DELTA_UPDATE_V2', 'SHARED_DELTA_UPDATE_V4');
  code = code.replace(
...
function patchUpdatePage(node) {
  ...
  htmlParam.value = mergeExistingConfluenceExpressionV4;
}
...
async function main() {
  ...
  try {
    ...
    const backupPath = path.join(backupDir, `workflow_${workflowId}_before_shared_doc_delta_update_v4_${stamp}.json`);
    ...
    console.log(JSON.stringify({
      ...
      backupPath,
      patched: [
        'shared delta update V4 prompt instructions',
        ...
        'pre-publish guards for duplicate headers, missing preserved sections, incomplete documents, and dropped coverage'
      ]
    }, null, 2));
  } finally {
    db.close();
