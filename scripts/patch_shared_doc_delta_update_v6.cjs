...
const mergeExistingConfluenceExpressionV6 = String.raw`={{ (() => {
  ...
  const extractHeadingSections = (html) => {
    ...
    for (let i = 0; i < matches.length; i += 1) {
      ...
      const next = matches.slice(i + 1).find(candidate => {
        ...
        return Boolean(canonicalForKey(sectionKey(candidate.title)));
      });
...
function patchPromptLibrary(node) {
  ...
  code = code.replace('SHARED_DELTA_UPDATE_V2', 'SHARED_DELTA_UPDATE_V6');
  code = code.replace('SHARED_DELTA_UPDATE_V4', 'SHARED_DELTA_UPDATE_V6');
  code = code.replace('SHARED_DELTA_UPDATE_V5', 'SHARED_DELTA_UPDATE_V6');
  code = code.replace(
...
function patchUpdatePage(node) {
  ...
  htmlParam.value = mergeExistingConfluenceExpressionV6;
}
...
async function main() {
  ...
  try {
    ...
    const backupPath = path.join(backupDir, `workflow_${workflowId}_before_shared_doc_delta_update_v6_${stamp}.json`);
    ...
    console.log(JSON.stringify({
      ...
      backupPath,
      patched: [
        'shared delta update V6 prompt instructions',
        ...
        'section-aware final body merge for shared update only, preserving nested subsection content',
        'update-mode table normalization and truncated-table guard',
        'pre-publish guards for duplicate headers, missing or content-thin preserved sections, incomplete documents, truncated output, malformed tables, and dropped coverage'
      ]
    }, null, 2));
  } finally {
    db.close();
