...
const mergeExistingConfluenceExpressionV7 = String.raw`={{ (() => {
...
function patchPromptLibrary(node) {
  ...
  code = code.replace('SHARED_DELTA_UPDATE_V2', 'SHARED_DELTA_UPDATE_V7');
  code = code.replace('SHARED_DELTA_UPDATE_V4', 'SHARED_DELTA_UPDATE_V7');
  code = code.replace('SHARED_DELTA_UPDATE_V5', 'SHARED_DELTA_UPDATE_V7');
  code = code.replace('SHARED_DELTA_UPDATE_V6', 'SHARED_DELTA_UPDATE_V7');
  code = code.replace(
...
function patchUpdatePage(node) {
  ...
  htmlParam.value = mergeExistingConfluenceExpressionV7;
}
...
async function main() {
  ...
  try {
    ...
    const backupPath = path.join(backupDir, `workflow_${workflowId}_before_shared_doc_delta_update_v7_${stamp}.json`);
    ...
    console.log(JSON.stringify({
      ...
      backupPath,
      patched: [
        'shared delta update V7 prompt instructions',
