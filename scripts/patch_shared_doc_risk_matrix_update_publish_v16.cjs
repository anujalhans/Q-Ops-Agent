...
function patchConverterNode(node) {
  ...
  const before = "let html = sanitizeSourceMetadata(String(source || ''))";
  const after = "let html = sanitizeSourceMetadata(String(source || '').replace(/<br\\s*\\/?>/gi, '\\n'))\n    .replace(/<br\\s*\\/?>/gi, '\\n')";
  if (code.includes(before)) {
    code = code.replace(before, after);
    patches += 1;
  }

  code = code.replace(/shared-final-validation-v14/g, 'shared-final-validation-v16');

  node.parameters.jsCode = code;
  return patches;
}

function patchSectionKeyInCode(code) {
  const before = "    .replace(/&/g, ' and ')\n    .replace(/[^a-z0-9]+/g, ' ')\n    .trim();";
  const after = "    .replace(/&/g, ' and ')\n    .replace(/[^a-z0-9]+/g, ' ')\n    .replace(/\\btop\\s+\\d+\\s+critical\\b/g, 'top critical')\n    .trim();";
  if (!code.includes(before) || code.includes("\\btop\\s+\\d+\\s+critical\\b")) {
    return { code, patched: false };
  }
  return { code: code.replace(before, after), patched: true };
}
...
function patchUpdateNode(node) {
  ...
  let patches = 0;
  const result = patchSectionKeyInCode(value);
  value = result.code;
...
(async () => {
  ...
  try {
    ...
    const backupPath = path.join(backupDir, `workflow_${workflowId}_before_shared_doc_risk_matrix_update_publish_v16_${stamp}.json`);
    ...
    if (entityPatches.updatePatches < 1 || historyPatches.updatePatches < 1) {
      throw new Error(`Update section-key patch incomplete: ${JSON.stringify({ entityPatches, historyPatches })}`);
    }
